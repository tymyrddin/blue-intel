Threat Intelligence
===============================================

Many forms of attacks are common today, including zero-day exploits, malware, phishing, on-path (man-in-the-middle)
attacks, and denial of service attacks. Different ways of attacking computer systems and networks constantly evolve
as attackers find new vulnerabilities to exploit.

Cyber threat intelligence (CTI) helps stop or mitigate an attack that is in progress. The more an
IT team understands about an attack, the better they will be able to make an informed decision about how to counter it.

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: CTI News

   docs/cti/README.md
   docs/cti/maps.md
   docs/cti/feeds.md
   docs/cti/blogs.md
   docs/cti/reports.md
   docs/cti/markets.md

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: Standards

   docs/standards/README.md
   docs/standards/taxii.md
   docs/standards/stix.md
   docs/standards/trees.md

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: Frameworks

   docs/frameworks/README.md
   docs/frameworks/lifecycle.md
   docs/frameworks/pyramid.md
   docs/frameworks/ckc.md
   docs/frameworks/mitre.md
   docs/frameworks/ukc.md
   docs/frameworks/diamond.md

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: On-line resources

   docs/analysis/README.md
   docs/analysis/reverse.md
   docs/analysis/ioc.md
   docs/analysis/phishing.md

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: OSINT Tools

   docs/osint/README.md
   docs/osint/urlscan.md
   docs/osint/abuse.md
   docs/osint/phishtool.md
   docs/osint/talos.md

.. toctree::
   :maxdepth: 1
   :includehidden:
   :caption: OpenCTI

   docs/opencti/README.md
   docs/opencti/data.md
   docs/opencti/dash.md
   docs/opencti/caddywiper.md

.. toctree::
   :caption: Links

   Blue Team <https://tymyrddin.github.io/blue/>
